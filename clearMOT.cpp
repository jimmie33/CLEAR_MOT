...
using namespace std;
//#define  _PETS01_
void C_Mot::dealWith(vector<ResultWindow> gt,vector<ResultWindow> hp,double thresh)
{
	frame_count++;
...
#endif
					if ((*hp_it).ID==last_match && getDistance((*hp_it),(*gt_it))<thresh)
					{
						//match found
						match_count++;
		...
		for (int i=0;i<matrix.rows();i++)
		{
			for (int j=0;j<matrix.columns();j++)
			{
				if (j<hp_size)
				{
					...
					matrix(i,j)=dis<thresh ? dis:INFINITY;
				}
				else
				{
					matrix(i,j)=2.0;
