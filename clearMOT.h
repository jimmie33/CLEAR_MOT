...
public:
	C_Mot():frame_count(0),error_sum(0),M_sum(0),FP_sum(0),IS_sum(0),match_count(0),gt_count(0),sta_writer(STA_FILE,ios::out),tra_count(0){}
	~C_Mot()
	{
		sta_writer.close();
	}
	void dealWith(vector<ResultWindow> gt,vector<ResultWindow> hp,double thresh=THRESH_VALIDATION);
